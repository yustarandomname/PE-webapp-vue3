...
const fetchAmount = 3; // TODO: for production, go back to 10
const list = ref<Post[]>([]); // list of posts in memory
const itemsToLoad = ref(fetchAmount); // items that are currently being loaded
const reachedBottom = ref(false); // if there are no more posts to load
const itemsBefore = ref(0); // number of items between now and the last fetch
