...
const displayName = computed(() => {
  if (userData.value)
    return props.fullName ? userData.value.fullName : userData.value.name_first;

  if (groupData.value) return groupData.value.name;
...
const openDetail = (event) => {
  ...
  else if (groupData.value) $setDetail({ group: groupData.value });
};

const loadUserData = async () => {
  if (!props.userId) return;
  const data = await UserData.getUserById(props.userId);
  userData.value = data;

  if (!data?.has_photo) return;
  const profileUrl = await data.getProfileImage();
  src.value = profileUrl ? profileUrl : src.value;
};

const loadGroupData = async () => {
  if (!props.groupId) return;
  const data = await GroupData.getGroupById(props.groupId);
...
onMounted(() => {
  ...
  else if (props.groupId) loadGroupData();
});
